...
- **`EncryptionService.startBootstrap` returns a `Bootstrap` object but the call chain ignores it.**
  `lib/src/encryption/encryption_service.dart:206-226` wires `onUpdate` correctly. `lib/src/widgets/encryption/post_login_setup_checker.dart:124-135` does receive it; OK. But `setupRequirement` is queried **before** `_refreshCrossSigningStatus` finishes on the very first sync — see the `Future.delayed(500ms)` workaround at line 85, which is a smell. Better: have `_refreshCrossSigningStatus` return a `Completer` and let callers `await` it.
